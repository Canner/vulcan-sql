...
import { Semantic } from '../../src/lib/semanticizer';
...
describe('Integration Test: Tokenize and Schema Integration', () => {
  it('should tokenize the input data and integrate with the schema', () => {
    const input = `
      ...
      model User @sql("SELECT * FROM Employee") @desc("User model") @pre_agg('5d') {
      ...
      Metric PostCount @model(Post) @pre_agg('1y') {
    ...
    expect(result.metrics[0].preAggregated).toBeUndefined();

    expect(result.metrics[1].timeGrain[0].dateParts).toEqual(['DAY', 'MONTH']);
    expect(result.metrics[1].preAggregated).toEqual(true);
    ...
    expect(result.models[0].preAggregated).toEqual(true);
    expect(result.models[0].refreshTime).toEqual('5d');

    expect(result.models[1].preAggregated).toBeUndefined();

...
You could use basic SQL Syntax to create an SQL file and define Data API by defining [API Schema](./configuration/api-schema) to let create the API endpoint and send the request to run your SQL file for querying data.
...
Second, define your `youth_users.sql` API entry point by [API Schema](./configuration/api-schema) for creating the API:
...
You could use `{{ context.params.[request-parameter] }}` to get the request parameter you defined it in [API Schema](./configuration/api-schema), the `[request-parameter]` should replace to your request parameter which is the same as the `fieldName` under the `request` of API schema , we use the basic SQL syntax sample:

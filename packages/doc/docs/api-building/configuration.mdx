...
types:
  - RESTFUL
# The external extension modules you wouldl like to use in your Vulcan project
extensions:
...
Although the above has shown 3 default configs, actually not only these built-in extension configs, you do not see here because VulcanSQL uses the default value of other extensions which not appeared it.

Otherwise, each extension has different config options, so we will talk about each extension in detail and in each extension chapter.

For the above config, if you are interested, you could see [Data Source Profile](./configuration/data-source-profile), Rate Limit (TBD, link), Enforce HTTPS (TBD, link), Response Format(TBD, link), and [Access Control](./access-control) first.
...
extensions:
  ext-module-name1: @vulcan-sql/xxx-module # installed Vulcan module

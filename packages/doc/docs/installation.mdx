...
If you have NodeJS (≥ 16) installed, you can simply install Vulcan CLI globally via NPM.
...
If you have [Docker Desktop](https://www.docker.com/products/docker-desktop/) installed, you can run Vulcan CLI directly.
...
1. Initialize a Vulcan project. In an empty folder, run the following command:
...
We need some credentials to connect to your data warehouses, which we call “profiles”. You need to set up at least one profile to let Vulcan work. Please check [Access Control](./api-building/access-control) for further information.

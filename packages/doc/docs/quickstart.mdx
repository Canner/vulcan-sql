...
2. Set the alias for `vulcan`.
   ...
   alias "vulcan-install"="vulcan-install"="docker run -it --rm -p 3000:3000 -v ${PWD}:/usr/app --entrypoint npm ghcr.io/canner/vulcan-sql/cli:dev install"
   ```
...
4.  Open the API document with URL: [http://localhost:3000/doc](http://localhost:3000/doc)
    ...
    ![the screenshot of Vulcan API document](./img/doc-screenshot.png)
...
3.  Throw error when user not found. We can let our API better by showing an accurate error message.
    ...
    The block `{% req user %} ... {% endreq %}` is a [query block](./api-building/sql-builder) which tells Vulcan that we want to save the query result to `user` variable instead of outputting as responses.
...
5.  The last step: add a [sample request](./api-building/api-document#set-sampler). Vulcan is unable to describe our API responses until we give it a sample request. When you open the [API document](http://localhost:3000/doc#/paths/~1customer/get), you'll see nothing has been described yet.

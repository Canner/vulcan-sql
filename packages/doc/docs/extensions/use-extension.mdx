...
VulcanSQL provides some external extension package for you could use it optionally, please see the [@vulcan-sql/](https://www.npmjs.com/search?q=%40vulcan-sql) prefix npm packages.

:::info
Not including `@vulcan-sql/core`, `@vulcan-sql/build` and `@vulcan-sql/serve` packages, this packages have installed globally when you installing `@vulcan-sql/cli`.
:::
...
Some extension options maybe rely on internal or built-extensions config options and set options under internal or built-extensions config options. We should check each document of the extensions npm package.

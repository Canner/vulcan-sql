...
:::info
Parsing SQL templates and generating AST are low-level operations of VulcanSQL compiler, it allows you to access the parser directly, it's the only way to add custom tags now, but we're planning to make the high-level components to create custom tags, please follow [Issue #115](https://github.com/Canner/vulcan-sql/issues/115).
...
You can use TagBuilder to add custom tags and control the rendering logic.

## How TagBuilder works

You'll need to claim some names first, when the compiler finds a matched name, it calls `parse` function of a specified tag builder, the tag builder takes the control, parses the template, then gives the control back to the compiler.
...
Assme that we're parser the following SQL:
...
2. When it found target tag names, that is, `mask` in this example. It gives the control to tag builder.

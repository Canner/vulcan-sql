...
After you finish your developed extension, you may would like to test your extension is workable and debug fault.

In this time you could do it by two ways:

## Local Testing

The local testing means you use the extension in the VulcanSQL by through npm link to test your extension locally.

Please use [npm link](https://docs.npmjs.com/cli/v8/commands/npm-link) to link your extension be an npm package locally.

After you linked, set the extension config in `vulcan.yaml`, same like our other extensions:
...
Finally, test yor extension by `vulcan start` directly to check.
...
- Filter: [FilterBuilder](./vulcan-sql-core/filter-builder) and [FilterRunner](./vulcan-sql-core/filter-runner)
- Tag: [TagBuilder](./vulcan-sql-core/tag-builder) and [TagRunner](./vulcan-sql-core/tag-runner)

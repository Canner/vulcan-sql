...
## Why Data Warehouse is not suitable for serving web applications
...
## VulcanSQL as a low-latency, high-concurrent Data API Application

VulcanSQL has emerged with the aim of helping users browse data faster and more conveniently, positioning itself as a low-latency, high-concurrent Data API Application. I will now describe why VulcanSQL is more suitable for serving Data APIs compared to a data warehouse.
...
I'll run a VulcanSQL server and Using BigQuery as the data warehouse with [TPC-H](http://www.tpc.org/tpch/) SF1 data inside, and Assuming we are building a API which will serve the daily revenue of the last 3 years

After [initialize](https://vulcansql.com/docs/develop/init) your VulcanSQL project, we can start building your API with sql templates

### Implement your API with sql templates

#### Step 1. Write your API

Under the "sql" folder, let create a `daily_revenue.sql` file with content, you will write your business logic here.

For example, I will serve a API that users can retrieve the daily revenue in a range period.
...
Under the "sql" folder, let create a `daily_revenue.yaml` file to configured your API
...
We've defined a API with path `/daily_revenue` that accepts query parameters. In the `cache` settings, we will retrieve aggregated data from BigQuery, store it in DuckDB, and refresh the cache on a daily basis.

#### Step 3. Configured the VulcanSQL project

In this example, We will connect to BigQuery and DuckDB, and using caching layer.
Let's configured your `vulcan.yaml` file to use BigQuery & DuckDB driver and setting DuckDB as our caching layer
...
In this case, we stored the aggregated data that retrieve from the data warehouse and turn a sql template into Data APIs.
...
With VulcanSQL, I see the respond time is still good with 100 concurrent users
...
And for 300 concurrent users, I see the respond time is grow to 1s
...
In this post, I've tried making sense that data warehouse is not suitable of serving web applications.
...
I've love to hear your feedback about VulcanSQL in our [Discord](https://discord.com/invite/ztDz8DCmG4) channel.

...
export class AuthHelper {
  ...
  public login = async (combination: {username: string, password: string}): Promise<{
    accessToken: string;
    refreshToken: string;
    profile: UserProfile;
  }> => {
    ...
    const refreshToken = jwt.sign({ session, profile }, this.refreshTokenSecret);
    await this.tokenStorage.setToken(session, apiToken);
    return {accessToken, refreshToken, profile};
  }

  public auth = async (accessToken: string): Promise<{apiToken: string, session: string, profile: UserProfile}> => {
    try {
      if (!accessToken) {
        throw errorCode.UNAUTHORIZED_REQUEST;
      }

      const { session, profile } = jwt.verify(accessToken, this.tokenSecret) as TokenPayload;
  ...
  public refreshToken = async (refreshToken: string): Promise<{
    accessToken: string;
    refreshToken: string;
    profile: UserProfile;
  }> => {
    const { session, profile } = jwt.verify(refreshToken, this.refreshTokenSecret) as TokenPayload;
    const newAccessToken = jwt.sign({ session, profile }, this.tokenSecret);
    const newRefreshToken = jwt.sign({ session, profile }, this.refreshTokenSecret);
    return {
  ...
  public logout = async (accessToken: string): Promise<void> => {
    try {
      const { session } = jwt.verify(accessToken, this.tokenSecret) as TokenPayload;
